...
class BatteryEnv(gym.Env):
    ...
    def reset(self):
        ...
        self.time_step = 0
        observation = np.array([self.prices_day_1[self.time_step], self.SoC, self.profits], dtype=np.float32)
        self.profits_history = [self.profits]  # Initialize profits history for current episode
...
def loop_agent(new_model, env_test, n_episodes):
    ...
    for i in range(n_episodes):
        ...
        profits_histories.append(env_test.profits_history)
    ...
    # Find the index of the best reward run
    best_reward_index = np.argmax(rewards)
    # Create a dictionary containing the best reward and profits history of the corresponding run
    best_reward_run = {'best_reward': rewards[best_reward_index],
                       'profits_history': profits_histories[best_reward_index]}

    return [best_reward_run, profits_histories]


# Loop the agent over multiple episodes and plot the rewards
[best_reward_run, profits_histories] = loop_agent(new_model, env_test, n_episodes=15)
...
def agent_run_evaluation(model, env):
    ...
    best_reward = max(env.profits_history)
    best_reward_index = env.profits_history.index(best_reward)
    best_reward_profits_history = profits_histories[best_reward_index]

    plt.plot(best_reward_profits_history)
    plt.xlabel('Time steps')
    plt.ylabel('Profits')
    plt.title('Profits over time (highest reward run)')
    plt.show()
